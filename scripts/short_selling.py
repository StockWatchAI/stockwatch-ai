...
2026年8月10日ぶんで実測した様式は次のとおり。ただしこれに寄せて書かないこと。

    ファイル  YYYYMMDD_Short_Positions.xls（**旧形式のxls**。xlsxではない）
    シート    公表年月日の8桁（例 '20260810'）。1枚だけ
    6行目     見出し。上に表題と公表年月日が入るので行番号は決め打ちできない
    列        1:計算年月日 2:銘柄コード 3:銘柄名 5:商号・名称・氏名
              10:空売り残高割合 11:空売り残高数量
    残高割合  0.017 のような**小数**で入る（1.7%の意味）
    規模      970件の報告 / 634銘柄

様式を疑うときは --inspect を使う。認証情報を使わないので副作用が無い。
"""
